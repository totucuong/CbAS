...
BLOSUM = np.array([[
    3.9029, 0.6127, 0.5883, 0.5446, 0.8680, 0.7568, 0.7413, 1.0569, 0.5694,
    0.6325, 0.6019, 0.7754, 0.7232, 0.4649, 0.7541, 1.4721, 0.9844, 0.4165,
    0.5426, 0.9365
],
                   [
                   ...
                   [
                       ...
                       0.4204, 0.3550, 0.6535, 0.6423, 0.3491, 0.6114, 0.4390,
                       0.3796, 0.7384, 0.7406, 0.4500, 0.4342, 0.7558
                   ],
                   [
                   ...
                   [
                       ...
                       0.4930, 13.5060, 0.3263, 0.3807, 0.7789, 0.5841, 0.6520,
                       0.4729, 0.7367, 0.5575, 0.4441, 1.7979, 0.3394
                   ],
                   [
...
def read_gfp_data(path=None, df_save_file=None):
    ...
    for line in f:
        if i == 0:
            ...
            cols = ['nucSequence', 'numNucMutations', 'numAAMutations'
                    ] + cols[2:]
            i += 1
            continue
        else:
...
def get_balaji_predictions(preds, Xt):
    ...
    var_star = (1 / M) * (np.sum(variances, axis=0) +
                          np.sum(means**2, axis=0)) - mu_star**2
...
def get_experimental_X_y(random_state=1,
    ...
    if return_test:
        ...
        X_train, gt_train = partition_data(X,
                                           y_gt,
                                           percentile=20,
                                           train_size=train_size,
                                           random_state=random_state,
